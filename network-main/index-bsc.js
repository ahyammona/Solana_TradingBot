    ...
    if(balanceOfPair > 0){
      ...
      //listen to buys
       erc20.on("Transfer", async(from,to,amount)=>{ 
            if(to == addressPair && trade == true){  //five
                   {
                     ...
                     if(tokenA == target && potCount < 1){
                        ...
                        potCount += 1;
                     }else if(lastbuys[count-2] === addressPair ){
                   ...
                   //    await swap.buy(token); 
                        ...
                        count = count + 1;
                        lastbuys.push(addressPair);
                        
                        profitHit = true;
                        await telegram.bot.sendMessage(msgId,`Purchased ${token} with Pair ${five.one_x_fiveLP}`)
                        //buy
                        await telegram.bot.sendMessage(msgId,
